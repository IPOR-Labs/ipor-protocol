...
contract("IporConfiguration", (accounts) => {
    ...
    before(async () => {
        tokenUsdt = await UsdtMockedToken.new(
            testUtils.TOTAL_SUPPLY_6_DECIMALS,
            6
        );
        tokenUsdc = await UsdcMockedToken.new(
    ...
    it("should set IporAssetConfiguration for supported asset", async () => {
        ...
        assert(
            iporAssetConfigurationAddress ===
                actualIporAssetConfigurationAddress,
    ...
    it("should use Timelock Controller - simple case 1", async () => {
        ...
        const calldata = await iporConfiguration.contract.methods
            .setMilton(fnParamAddress)
            .encodeABI();
        ...
        assert(
            fnParamAddress === actualMiltonAddress,
    ...
    it("should set Milton Publication Fee Transferer", async () => {
        ...
        //when
        await iporConfiguration.setMiltonPublicationFeeTransferer(mockAddress, {
            from: userOne,
        });
    ...
    it("should NOT set Milton Publication Fee Transferer when user does not have MILTON_PUBLICATION_FEE_TRANSFERER_ROLE role", async () => {
        ...
        await testUtils.assertError(
            //when
            iporConfiguration.setMiltonPublicationFeeTransferer(mockAddress, {
                from: userOne,
            }),
