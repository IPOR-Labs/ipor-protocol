...
contract MiltonMaintenanceTest is Test, TestCommons, MiltonUtils, MiltonStorageUtils, JosephUtils, IporOracleUtils, DataUtils, SwapUtils, StanleyUtils {
	...
	function testShouldNotPauseSmartContractSpecificMethodsWhenPaused() public {
		...
		vm.prank(_userTwo);
		mockCase0MiltonDai.itfOpenSwapReceiveFixed(block.timestamp, 10000*Constants.D18, 1*10**16, 10*Constants.D18); 
		vm.prank(_userTwo);
	...
	function testShouldNotSendEthToMiltonDaiUsdctUsdc() public payable {
		...
		// when 
		vm.expectRevert("Transaction reverted: function selector was not recognized and there's no fallback nor receive function");
