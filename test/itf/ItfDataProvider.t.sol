...
contract ItfDataProviderTest is TestCommons, DataUtils {
    ...
    function setUp() public {
        ...
        _iporOracle = getIporOracleAsset(
            _userOne,
    ...
    function testShouldCollectDataFromIporOracleForItf() public {
        ...
        // when
        ItfDataProviderTypes.ItfIporOracleData memory iporOracleData = _itfDataProvider
            .getIporOracleData(block.timestamp, address(_usdcMockedToken));
        ItfDataProviderTypes.ItfMiltonData memory miltonData = _itfDataProvider.getMiltonData(
            block.timestamp,
            address(_usdcMockedToken)
        );
        ItfDataProviderTypes.ItfMiltonStorageData memory miltonStorageData = _itfDataProvider
            .getMiltonStorageData(address(_usdcMockedToken));
        ItfDataProviderTypes.ItfMiltonSpreadModelData
            memory miltonSpreadModelData = _itfDataProvider.getMiltonSpreadModelData(
                address(_usdcMockedToken)
            );
        ...
        assertEq(
            iporOracleData.accruedExponentialMovingAverage,
            TestConstants.TC_5_EMA_18DEC_64UINT
        );
        ...
        assertEq(
            miltonData.wadLiquidationDepositAmount,
        ...
        assertEq(
            miltonSpreadModelData.payFixedRegionTwoSlopeForMeanReversion,
        ...
        assertEq(
            miltonSpreadModelData.receiveFixedRegionOneSlopeForMeanReversion,
            999999997304907264
        );
        assertEq(miltonSpreadModelData.receiveFixedRegionTwoBase, -250000000000000);
        assertEq(
            miltonSpreadModelData.receiveFixedRegionTwoSlopeForVolatility,
            -600000000289261748224
        );
        assertEq(
            miltonSpreadModelData.receiveFixedRegionTwoSlopeForMeanReversion,
            TestConstants.ZERO_INT
        );
