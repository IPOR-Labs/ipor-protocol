...
contract ForkAmmStEthOpenSwapsTest is TestForkCommons {
    ...
    function prepareTotalAmount(
        ...
        uint256 availableAmount = IporMath.division(
            collateral *
                (1e18 + IporMath.division(IporMath.division(leverage * openingFeeRate * tenorInDays, 365), 1e18)),
            1e18
        );
        uint256 wadTotalAmount = availableAmount + wadLiquidationDepositAmount + iporPublicationFeeAmount;
        return wadTotalAmount;
    }

    function prepareTotalAmount2(
        uint256 collateral,
        uint256 leverage,
        uint256 openingFeeRate,
        uint256 tenorInDays,
    ...
    //Wrong: 50494452054794520548
    //Correct: 50494452054794520550

//250130890410958904000000n

    // collateral 70, leverage 10 - not working w USDT/USDC
    // collateral 70, leverage 10 - working w DAI 105026849315068493120
    // collateral 1, leverage 250 - working
    // collateral 0.1, leverage 250 - working

    function testShouldOpenPositionStEthForStEth28daysPayFixedRoundingCase1() public {
        //given
        _init();
        _createAmmOpenSwapServiceStEthMainnet();
        _updateIporRouterImplementation();

        address user = _getUserAddress(22);
        _setupUser(user, 1000 * 1e18);

        uint256 collateral = 70 * 1e18;
        uint256 leverage = 10;

        uint256 calculatedTotalAmount = prepareTotalAmount(
            collateral,
            leverage * 1e18,
            5e14,
            28,
        ...
        console2.log("calculatedTotalAmount=", calculatedTotalAmount);

        uint256 totalAmount = calculatedTotalAmount;
        ...
        console2.log("collateral=", swap.collateral);
        console2.log("notional=", swap.notional);
        ...
        assertEq(collateral, swap.collateral, "swap.collateral");
        assertEq(leverage * collateral, swap.notional, "swap.notional");

    }

    function testShouldOpenPositionStEthForStEth28daysPayFixedRoundingCase2() public {
        //given
        _init();
        _createAmmOpenSwapServiceStEthMainnet();
        _updateIporRouterImplementation();
        ...
        uint256 collateral = 1 * 1e18;
        uint256 leverage = 250;
    ...
    function testShouldOpenPositionStEthForStEth28daysPayFixedRoundingCase3() public {
        //given
        _init();
        _createAmmOpenSwapServiceStEthMainnet();
        _updateIporRouterImplementation();
        ...
        assertEq(collateral, swap.collateral, "swap.collateral");
        assertEq(250 * collateral, swap.notional, "swap.notional");
