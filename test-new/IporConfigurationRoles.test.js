...
const IPOR_ASSETS_ADMIN_ROLE = keccak256("IPOR_ASSETS_ADMIN_ROLE");
    ...
    const roles = [
        ...
        {
            name: "MILTON_SPREAD_STRATEGY_ROLE",
            adminRole: keccak256("MILTON_SPREAD_STRATEGY_ADMIN_ROLE"),
            role: keccak256("MILTON_SPREAD_STRATEGY_ROLE")
        },
        {
    ...
    const rolesNotGrant = [
        ...
        {
            name: "MILTON_SPREAD_STRATEGY_ADMIN_ROLE",
            role: keccak256("MILTON_SPREAD_STRATEGY_ADMIN_ROLE"),
            code: "0xa49807205ce4d355092ef5a8a18f56e8913cf4a201fbe287825b095693c21775"
        },
        {
...
describe('IporConfigurationRoles', () => {
    ...
    itParam("should grant and revoke ${value.name}", roles, async function (value) {
        //given
        ...
        let hasAdminRole = await iporConfiguration.hasRole(adminRole, userOne.address);
        expect(hasAdminRole).to.be.false;

        await iporConfiguration.grantRole(adminRole, userOne.address);

        hasAdminRole = await iporConfiguration.hasRole(adminRole, userOne.address);
        expect(hasAdminRole).to.be.true;

        let hasRole = await iporConfiguration.hasRole(role, userTwo.address);
        expect(hasRole).to.be.false;

        await iporConfiguration.connect(userOne).grantRole(role, userTwo.address);

        hasRole = await iporConfiguration.hasRole(role, userTwo.address);
        expect(hasRole).to.be.true;

        await iporConfiguration.connect(userOne).revokeRole(role, userTwo.address);
      ...
      itParam("should not be able to grant role ${value.name}", rolesNotGrant, async function (value) {
        ...
        await expect(iporConfiguration.connect(userOne).grantRole(role, userTwo.address))
