...
module.exports = async function (deployer, _network, addresses) {
    ...
    if (process.env.IPOR_OWNER_ADDRESS) {
        const iporOwnerAddress = process.env.IPOR_OWNER_ADDRESS;
        ...
        const compoundStrategyUsdtInstance = await StrategyCompoundUsdt.at(
            compoundStrategyProxyUsdt
        );
        const compoundStrategyUsdcInstance = await StrategyCompoundUsdc.at(
        ...
        const miltonFacadeDataProviderProxy = await func.getValue(
            keys.MiltonFacadeDataProviderProxy
        );
        ...
        console.log("WARNING! Transfer ownership SKIPPED, IPOR_OWNER_ADDRESS is not set!");
    }
